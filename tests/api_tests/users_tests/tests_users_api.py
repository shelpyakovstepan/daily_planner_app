...
@pytest.mark.parametrize(
    ...
    [(1, True, 403, 200), (100, True, 409, 409)],
)
async def test_change_admin_status(
    user_id,
    admin_status,
    status_code_for_non_admin,
    status_code_for_admin,
    authenticated_ac: AsyncClient,
    authenticated_ac_admin: AsyncClient,
):
    non_admin_user_request_response = await authenticated_ac.patch(
        "/auth/admin",
        params={"user_id": user_id, "admin_status": admin_status},
    )

    assert non_admin_user_request_response.status_code == status_code_for_non_admin

    admin_user_request_response = await authenticated_ac_admin.patch(
        "/auth/admin",
        params={"user_id": user_id, "admin_status": admin_status},
    )
...
async def test_get_me(
    ...
    assert (
        authorized_user_request_response.status_code == status_code_for_authorized_user
    )
